...
-- hds interface_end
ARCHITECTURE rtl OF first_key IS
	signal k1,k2 :std_logic_vector(63 downto 0);
	signal rest:unsigned (7 downto 0);
	type t_array is array (0 to 5) of std_logic_vector(63 downto 0);
...
if (rst = '1') then
	...
	if (rest = 1+count*5) then
		k1   <= k1(40 downto 0)&k1(63 downto 41); 
		k2   <= k2 xor kin ;
    	rest <= rest +1 ;

	elsif (rest = 2+count*5) then
    	k2   <= k2(22 downto 0) & k2(63 downto 23);
		rest <= rest +1 ;
	
	elsif (rest = 3+count*5) then
		k1   <=k1 xor k2 ;
		rest <= rest +1 ;
	
	elsif (rest = 4+count*5) then

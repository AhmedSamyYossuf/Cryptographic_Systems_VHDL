...
ARCHITECTURE rtl OF decryption11 IS
	...
	signal count,m,finish,p_invers:NATURAL;
BEGIN 
process(clk)
BEGIN
	if (count = 1) then 
		...
		--	en  <= '1';
			...
			ciphertext <= ciphertext_temp xor k1 ;
			finish <= finish +1;
		--	en  <= '0';		
		end if;
	end if ;
	...
	if (rst='1')then
		if (typ2 = '1') then 
			...
			rest	<= (others =>'0') after 1300 ns;
			count 	<= 0 			  after 1300 ns;
			m  		<= 0 			  after 1300 ns;
			finish 	<= 0 			  after 1300 ns;
			p_invers<= 0			  after 1300 ns;
			en 		<= '1' 			  after 1300 ns;	

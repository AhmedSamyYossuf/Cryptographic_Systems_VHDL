...
ARCHITECTURE rtl OF encre IS
	...
	type K_array is array (0 to 17) of std_logic_vector(63 downto 0);
	signal  k_All : K_array;
	signal count,m,finish,control:NATURAL;
BEGIN 
process(clk)
BEGIN
	if (count = 16) then 
...
---------------------------------------------------------------------------------------------
	...
	------------------------------------------12 to 15 ----------------------------------------------------
		...
		if (count <15) then
			test <=	  S_All(2) & S_All(1) & S_All(0)& S_All(3);
			S_All(0) <= S_All(3);
			S_All(1) <= S_All(0);
			S_All(2) <= S_All(1);
			S_All(3) <= S_All(2);
